...
string AVL::TraversePreOrder() const
{
...
string AVL::TraversePostOrder() const
{
	return TraversePostOrderInternal(m_Root);
}

string AVL::TraversePostOrderInternal(const NodeAVL* node) const
{
	if (node != nullptr)
	{
		std::ostringstream oss;
...
NodeAVL* AVL::Insert(const produto& data)
{
	if (IsEmpty())
	{
		m_Root = new NodeAVL(data);
		return m_Root;
	}

	return InsertInternal(m_Root, nullptr, data.getCodigo, data);
}

NodeAVL* AVL::InsertInternal(NodeAVL* node, NodeAVL* parent, int id, const produto& data)
{
	if (node == nullptr)
		node = new NodeAVL(data, parent);
	else if (node->GetID() == id)
		return nullptr; 
	else if (node->GetID() > id)
...
string AVL::TraverseInOrderInternal(const NodeAVL* node) const
{
	if (node != nullptr)
	{
		ostringstream oss;
...
string AVL::TraverseInOrder() const
{
